...
class FoundationPose_ROS:
    def __init__(self, config_file):
        ...
        # Create pose publishers for visualization in RViz
        self.pose_publishers = {}
        for obj_name in self.mesh_files.keys():
            ...
            try:
                ...
                if pose_result is not None:
                    ...
                    if hasattr(pose_result, 'shape') and pose_result.shape == (4, 4):
                        ...
                        if mapped_name in self.pose_publishers:
                            pose_stamped = PoseStamped()
                            pose_stamped.header.stamp = rospy.Time.now()
                            pose_stamped.header.frame_id = "head_rgbd_sensor_rgb_frame"  # Adjust frame_id as needed
                            pose_stamped.pose = pose_msg
                            self.pose_publishers[mapped_name].publish(pose_stamped)
                            print(f"  -> Published pose to /pose_estimator/foundationpose/{mapped_name}/pose")
